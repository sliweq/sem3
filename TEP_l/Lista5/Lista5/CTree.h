...
public:
	...
	void operator=(CTree<T>&& ctree_new_val);
...
//MS
template <typename T>
inline CTree<T>::CTree<T>(CTree<T>&& ctree_new_val) {
	//std::cout << "Konstruktor kopiuj¹cy move";
	root_node = ctree_new_val.root_node;
	ctree_new_val.root_node = NULL;
}

template <typename T>
void CTree<T>::operator=(CTree<T>&& ctree_new_val) {
	if (root_node != NULL) {
		delete root_node;
	}
	root_node = ctree_new_val.root_node;
...
inline CTree<T> CTree<T>::operator+(const CTree<T>& ctree_new_val) {
	...
	//result_ctree = *this;
	(*result_ctree.root_node).copy_node_value(ctree_new_val.root_node);

	(*result_ctree.root_node).find_number_node(ctree_new_val.root_node);
	return std::move(result_ctree);
	//return result_ctree;
};
