...
COptimizer::COptimizer(CLFLnetEvaluator &cEvaluator)
	: c_evaluator(cEvaluator), cGeneticAlgorithm(constPopSize, constCrossProb, constMutProb, &c_evaluator){
...
void COptimizer::vRunIteration(){
	...
	v_candidate = cGeneticAlgorithm.runIteration()->getGenotype();
	...
	if (d_candidate_fitness > d_current_best_fitness){
		...
		cout << newBestFittnesMess << d_current_best_fitness << endl;
	}//if (d_candidate_fitness > d_current_best_fitness)
}//void COptimizer::vRunIteration()
